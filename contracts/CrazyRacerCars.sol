 ...
 * @dev Soulbound ERC-721: 1 NFT per car type per wallet, non-transferable
 * Car types: 0,1,2 = free | 3,4,5 = paid (0.00015, 0.0002, 0.00025 ETH)
 */
contract CrazyRacerCars is ERC721, Ownable, ReentrancyGuard {
    uint256 public constant MAX_CAR_TYPES = 6;
    uint256 private _nextTokenId;

    // carType => price in wei (0 = free)
    ...
    ) ERC721("Crazy Racer Cars", "CRC") Ownable(msg.sender) {
        treasury = _treasury;
    ...
    function balanceOfCarType(address owner, uint8 carType) external view returns (uint256) {
        return hasMinted[owner][carType] ? 1 : 0;
    }
